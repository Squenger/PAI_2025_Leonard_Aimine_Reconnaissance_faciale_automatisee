...
shutil.rmtree('src/facial_recognition.egg-info', ignore_errors=True);
for f in glob.glob('src/facial_recognition/*.so'): os.remove(f);
for f in glob.glob('src/facial_recognition/*.c'): os.remove(f);
"""
...
def tests_compiled(session: nox.Session) -> None:
    ...
    session.install("-e", ".", env={"FACIAL_RECOGNITION_COMPILE_MYPYC": "1"})
